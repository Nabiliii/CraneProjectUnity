...
#pragma warning disable 0219
    ...
    public class Drive : BaseDrive
    {
        ...
        //write to file
        //private string logFileName = "position_and_speed_log.txt";
        ...
        {
            /*
            //new code to save pose and speed in another file
        ...
        // Define a dictionary to store logWriters for each object
        ...
        private void FixedUpdate()
        {
            ...
            /*
            // Log the current position and speed to the console
            ...
            if (Input.GetKeyDown(KeyCode.M))
            {
                ...
                {
                    // Set the flag to request a snapshot
            ...
            if (isSnapshotRequested)
            {
                ...
                string logMessage1 = index + "  " + gameObject.name + " ----> Position = " + CurrentPosition + "   , Speed = " + CurrentSpeed + "\n";
                string logMessage2 = index + " {" + CurrentPosition + ", " + CurrentSpeed + ", " + _currentdestination + "}\n";

                // Print logMessage1
                print(logMessage1);

                // Ensure the log message has the index and write it to the log file
                string indexedLogMessage = logMessage2;

                StreamWriter logWriter;

                // Check if a logWriter exists for this object, create if not
                if (logWriters.ContainsKey(gameObject.name))
                {
                    logWriter = logWriters[gameObject.name];
                }
                else
                {
                    // Create a new logWriter for this object
                    string customPath = "C:\\crane_ship\\crane_ship\\Assets\\Annotations";
                    string logFileName = gameObject.name + "_log.txt";
                    string logFilePath = Path.Combine(customPath, logFileName);
                    logWriter = new StreamWriter(logFilePath);

                    // Store the logWriter for this object
                    logWriters.Add(gameObject.name, logWriter);
                }

                logWriter.Write(indexedLogMessage);
                logWriter.Flush(); // Ensure data is written immediately to the file

                // Reset the flag after saving the object information
        ...
        void Update()
        {
            ...
            if (isSnapshotRequested)
            {
                ...
                // Construct the log messages
                string logMessage1 = index + "  " + gameObject.name + " ----> Position = " + CurrentPosition + "   , Speed = " + CurrentSpeed + "\n";
                string logMessage2 =  CurrentPosition + " ; " + CurrentSpeed +"\n";
        ...
        {
            // Close the log files for all objects
            foreach (StreamWriter logWriter in logWriters.Values)
            {
                logWriter.Close(); // Close the StreamWriter
            }
        }
